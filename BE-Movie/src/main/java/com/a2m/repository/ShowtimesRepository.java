...
public interface ShowtimesRepository extends JpaRepository<Showtimes, Long> {
    ...
    @Query("SELECT DISTINCT new ShowtimeResponse (st.movie.id, st.movie.name, st.movie.poster, st.movie.category.name, st.movie.premiereDate) FROM Showtimes st WHERE date(st.date) = :today AND st.isDelete <> true")
    List<ShowtimeResponse> today(Date today);

    @Query("SELECT DISTINCT new ShowtimeResponse (st.movie.id, st.movie.name, st.movie.poster, st.movie.category.name, st.movie.premiereDate) FROM Showtimes st WHERE st.isDelete <> true")
